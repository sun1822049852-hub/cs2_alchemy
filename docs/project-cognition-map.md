...
| Steam Guard 与 refresh token | `steam_account` 中的账号密码和 Guard-only maFile；`TokenStore` 中的 refresh token | `/api/accounts` 只投影 `has_steam_guard`、`has_refresh_token`；令牌码接口只投影当前码/周期/剩余秒数 | 联合绑定 `flow_id`、临时 refresh/access token、候选 Guard 数据、自动重连中/needs_attention | `AppAuthStore.updateSteamGuard()`、`steamGuardCoexistService`、`tokenRecoveryService`、`TokenStore` | 账号卡令牌管理、库存/GC/Web/市场/封禁等认证消费者 | 联合 flow 只在内存保留五分钟；先持久化新 refresh token，再失效旧 session；账号列表不得返回密码、maFile 或 secret |
...
| `steam.guard.coexist` | 账号卡“绑定令牌” -> coexist start -> 邮箱码 -> `AddAuthenticator` -> Steam App 五位码本地恒时校验 -> Guard 持久化 | Node HTTP request；flow 由 `accountViewerUsername` 绑定并校验 | `Requires2FA/DeviceCode/DeviceConfirmation` 与 AddAuthenticator `status=29` 均只提示已有令牌并销毁流程；不调用 finalize/remove/replace；成功响应不返回 maFile/secret | 2026-07-17 定向 service/route/UI 契约测试通过；真实 Steam 未验证 |
| `token.recovery` | 业务连接 -> 当前 refresh token -> 明确缺失/拒绝时账号密码+本地 TOTP 登录 -> TokenStore 写入校验 -> session invalidation -> 原操作重试一次 | Node session pool、refresh workflow 与 Web operation wrapper | 每账号 single-flight；网络错误不触发；恢复后再拒绝、密码/Guard 缺失或额外人工验证进入 `needs_attention` | 2026-07-17 定向 token/session/refresh/Web 测试通过；真实过期账号未验证 |
...
| `node_sidecar/src/steamGuardCoexistService.js`、`steamGuardSteamAdapter.js`、`steamGuardTokenService.js` | Auth / external account | 联合绑定内存状态机、唯一 AddAuthenticator 调用、Guard-only maFile 规范化、后端 TOTP/导出 | Steam Guard coexist/token routes、账号卡令牌管理 | 临时登录 token、候选 Guard、`steam_account.mafile_content` | `AppAuthStore.updateSteamGuard()`、安全 API 投影 | 不得恢复 finalize/remove/replace；真实 Steam App 共存流程仍需测试账号验证 |
| `node_sidecar/src/tokenRecoveryService.js`、`services/sessionPool.js` | Auth / recovery | refresh token 缺失/拒绝后的 single-flight 自动登录、持久化、会话失效和单次业务重试 | 库存刷新、GC Session、Web inventory、market/account tools | `TokenStore`、账号凭据、Guard TOTP、session pool | refresh runtime、Web operation wrapper、UI reconnecting/needs_attention | 网络失败不能误触发恢复；durable token write 失败不得发成功；不可递归恢复 |
