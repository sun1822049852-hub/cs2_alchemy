...
- `last verified`: `2026-05-30`（全项目审查覆盖）；VPK dev-only 打包边界小范围补验为 `2026-05-31`；地图结构补全和主工作区只读入口核对为 `2026-07-06`；VOM/POR 边界、权限 owner、库存传播边和军械库链路的定向静态核对为 `2026-07-13`；模拟配方、快速选材、批量 preset 编辑和 Web 库存前置检查的直接影响范围补验为 `2026-07-15`；模拟材料零磨损边界和即时卡片投影补验为 `2026-07-17`；Steam Guard-only 批量导入、重复覆盖、本地令牌删除和绑定后过期账号恢复链路补验为 `2026-07-18`；多标签页库存 SSE 生命周期与真实 `code=15` 手动重登分流补验为 `2026-07-19`；联合绑定独立进程、可重试验证码、已有本地 maFile 仍以 Steam 为远端事实源及 Guard-only 可选密码导入补验为 `2026-07-20`。
- `scope/coverage`: 覆盖当前主工作区 `C:/Users/18220/Desktop/cs2_alchemy` 的静态只读扫描、已审切片结论、2026-05-28 对 Skin DB 同步/C5 磨损来源链路的定向实现验证，以及 2026-05-30 项目审查修复的授权、账号范围、前端权限、批量炼金、运行态 JSON、代理、Admin Console、打包预检等直接影响区；另覆盖 2026-05-31 VPK dev-only 打包边界小范围补验。2026-07-06 只补齐地图结构、状态口径和主要业务文件职责导航；2026-07-13 只核对 VOM 分类、license/AppAuthStore owner 边界、inventory/component/craft/wallet 的高价值传播入口和军械库兑换源码/测试路径；2026-07-15 只覆盖模拟 picker/preset/export、craft-assist preset/display name、batch preset editor、ban SteamID fallback 和 Steam 网络前置检查的源码与定向测试；2026-07-17 只覆盖模拟材料选择、零相对磨损预测模式、预测失败 warning 和材料卡消费入口；2026-07-18 只覆盖 `codex/steam-guard-coexist` 中 Guard-only maFile 预检/保存、同名覆盖确认、本地 Guard 删除、未登录投影、首次连接前 profile 抑制，以及 EResult 15 的旧认证清理与 maFile 分流恢复。
- `validation method`: 静态路径核对、切片整理、定向 Node 测试、`admin_console npm test`、`node_sidecar` packaging preflight、2026-05-31 VPK CLI help；2026-07-06 只读核对 `AGENTS.md`、`git worktree list --porcelain`、当前主工作区 `git status --short`、现有地图和关键源码入口/服务导出；2026-07-13 对照最新 `verified-operation-map` skill 与 POR schema，并只读核对所列关键源码/测试定位及 workflow-state audit（exit `0`）；2026-07-15 运行 21 个直接/相邻 Node 测试文件和三个 `node --check`，并在 `127.0.0.1:8788` 浏览器运行态验证 preset 保存/重命名、受限模拟候选过滤、中文导出角色/范围、快速选材 preset 内容和 batch 仓库搜索浮层边界；2026-07-17 运行模拟派生产物、页面状态和完整 picker 交互测试，并以真实本地 catalog/predictor payload 验证零磨损 `infinite` 边界；2026-07-18 运行 Guard 定向测试、完整 `node_sidecar npm test`，并用临时数据库和合成账号在真实页面验证桌面/390px 两个弹窗、未登录徽标、确认层级、覆盖计数、取消汇总和横向溢出；2026-07-20 再次运行联合绑定路由定向测试和完整 `node_sidecar npm test`，默认 119 项全部通过；未连接真实 Steam。audit 只证明 workflow state 输入完整，不证明地图业务内容正确。
- `未覆盖`: 未检查其它 worktree 的源码和 diff；未读取真实账号、Steam session、真实 DB 内容或外部交易状态；未访问 Steam、BUFF、SteamDT、C5 等外部服务；未做全项目端到端 UI 运行态验证（仅完成 2026-07-15 直接影响范围的截图/DOM 验证、2026-07-17 模拟 picker 完整浏览器交互测试和 2026-07-18 Guard 导入/令牌管理局部运行态验证）；`node_sidecar npm test` 的 119 个默认测试已在 Guard 分支通过，但默认排除的浏览器交互与 pending specification 测试未纳入；未建立任何 POR；未完成全部状态传播边/消费者/失败旁路盘点；未判断主工作区既有未提交业务改动是否正确。
...
| Infrastructure / runtime | 路径常量、SQLite、凭据文件、代理、开发文件日志、Electron `userData`、打包 seed | 运行态文件、凭据和环境观察态；开发态和打包态可能不同 | `constants`、store/secret/proxy/session 相关模块；通用控制台与可选文件日志由 `logger` / `devFileLog` 统一分发和脱敏 | 应用启动、服务层读写、打包首启 | `csgo_skins.db`、`login_keys.json`、`client_config.json`、`schema_cache.json`、可选 `logs/dev/*.jsonl`、Electron `userData`、系统代理/DPAPI | 账号登录、库存和市场链路、打包产物、调试和审查 |
...
| 本地 user、super-admin 与 Steam 账号访问 | SQLite 中的本地 user/super-admin、`steam_account`、viewer binding 和 `app_setting` 迁移/全局 active 状态 | `/api/accounts`、本地 user display/super-admin 投影、`AccountStore` | session/viewer 解析、last-selected、账号表单状态 | `AppAuthStore`、`AccountStore`、`UiStateStore` | API `hasPermission()`、snapshot/refresh/trade/market/craft/ban 路由 | 当前控制面用户具备相应 permission code 时可访问全部本地 Steam 账号，viewer binding 不构成业务隔离；本地 super-admin 不绕过签名权限。`accounts.json` 只作为一次性迁移输入：仅补齐 SQLite 缺失账号，现有 SQLite 值优先，成功后删除旧文件并以 `app_setting` 标记完成；不得重新引入每次初始化重放旧账号的兼容读路径。ban 检测兼容读取 `steam_id64 || steam_id`，不能只用缺省的 `steam_id64` 判断未绑定 |
...
| Steam Guard 与 refresh token | `steam_account` 中的账号密码和 Guard-only maFile；`TokenStore` 中的 refresh token | `/api/accounts` 对具备 `accounts.write` 且通过 account scope 的前端投影已保存密码、`has_steam_guard`、`has_refresh_token`，并设置 `Cache-Control:no-store`；只读授权不投影密码；maFile/secret 始终不投影；Guard-only 无 token 账号显示“未登录” | 联合绑定 `flow_id`、临时 refresh/access token、候选 Guard 数据、批量导入预检/覆盖选择、自动重连中/needs_attention、无 maFile 的登录过期手动重登弹窗、`password_reentry_required` | `AppAuthStore.saveVerifiedSteamCredentials/updateSteamGuard()`、`create/overwrite/clearSteamGuard/clearSteamPassword`、`steamGuardCoexistService`、`steamGuardImportService`、`tokenRecoveryService`、`TokenStore` | 账号卡令牌管理、批量 maFile 导入、手动重登表单、绑定后自动重登、库存/GC/Web/市场/封禁等认证消费者 | 重登弹窗预填本地已保存密码是明确产品要求，不得误判为缺陷；账号密码阶段成功且 Steam 要求人工 Guard 码时，先用专用入口持久保存已验证账号密码再返回验证码阶段，已有账号只改密码，新账号只创建账号密码和当前本地用户绑定，不写 TokenStore、不改资料、不切换活动账号；验证码阶段隐藏并禁用账号/密码，只显示只读账号标识和唯一可编辑的 5 位 `A-Z0-9` 输入，前端过滤/转大写/截断且后端所有登录码入口严格复验，不显示邮箱说明、图案或无法访问邮箱帮助；Steam `InvalidPassword/code=5` 对本地已有账号显示“密码错误”，新增账号仍显示“账号或密码错误”，两者均保持 `reason=invalid_password`；批量导入不登录、不写 TokenStore；覆盖只改密码和 maFile；本地删除只清 maFile；密码拒绝只清 password；本地 maFile 登录由后端在密码阶段成功并收到 device-code 要求后生成/提交动态码，前端不接收该码；未知 Steam 密码失效码不得在没有证据时加入枚举 |
...
| 汰换模拟和预测 | 皮肤基础库、snapshot rows、算法输入 | `/api/craft/predict-outcomes`、`/api/simulation/tradeup/*` 输出 | 表单选择、preset、localStorage 草稿、导出表单 | `craftOutcomePredictor`、`tradeupSimulationService`、前端 preset 持久化入口 | 模拟页、炼金预览、导出到快速选材 | 已保存 preset 的普通保存覆盖原记录，重命名只走独立入口；导出时 English `name` 保持匹配真相，`display_name` 仅用于中文显示，主/辅角色和数量/磨损范围分别持久化；模拟链路不得触发真实账号动作 |
...
| `steam.guard.coexist` | 账号卡“绑定令牌” -> coexist start -> 邮箱码 -> `AddAuthenticator` -> Steam App 五位码本地恒时校验 -> Guard 持久化 -> 原状态为 `auth_invalid` 的已有账号调用 `/api/accounts/steam-guard/recover-login` | Node HTTP request；flow 由 `accountViewerUsername` 绑定并校验；恢复 API 再执行 `accounts.write` 与 account scope gate | `Requires2FA/DeviceCode/DeviceConfirmation` 与 AddAuthenticator `status=29` 均只提示已有令牌并销毁流程；不调用 finalize/remove/replace；成功响应不返回 maFile/secret；非过期账号不重建 token；密码失效先只清本地 password，持久化成功后才返回 `credential_state=password_reentry_required/password_cleared=true` | 2026-07-18 定向 service/route/UI 契约与恢复边界测试通过；真实 Steam 未验证 |
| `steam.guard.batch-import` | 本地 maFile `account_name`/Guard 字段 -> preflight 安全投影 -> ready 新增或用户勾选覆盖 -> `steam_account.password/mafile_content` | `steamGuardImportService` 统一二次校验；SSE 只发送账号名、文件名、状态和计数；浏览器独立重复窗口消费 | 同批可解析的同名项整组阻止；新增不登录、不写 TokenStore；覆盖只写 password+maFile并保留 profile/token/session/active；失败项不发成功，raw maFile/密码在关闭弹窗时清理 | 2026-07-18 route/store/UI 测试和桌面/390px 合成数据运行态通过；真实 maFile/生产数据库未验证 |
| `steam.guard.local-delete` | 账号卡令牌管理 -> 删除确认 -> `DELETE /api/accounts/steam-guard` -> 仅清 `steam_account.mafile_content` -> 账号卡重载 | `accounts.write` + account scope gate；`AppAuthStore.clearSteamGuard()` 单字段持久化；前端清 TOTP timer/恢复码内存 | 不调用 Steam finalize/remove/replace，不删除 password/refresh token/session/profile；数据库成功但列表刷新失败时明确提示“已删除但刷新失败”，不能误报后端删除失败 | 2026-07-18 route/store/UI 测试和合成账号确认弹窗运行态通过；未执行真实账号删除 |
| `token.recovery` | 业务连接、手动连接或绑定后恢复 -> 当前 refresh token -> 收到结构化 `EResult 15` 或真实消息信封 `AccessDenied code=15` 后先持久删除旧 token 并失效 Session Pool -> 有 maFile 时最多一次账号密码+本地 TOTP 登录，无 maFile 时直接手动登录 -> TokenStore 写入校验 -> 原操作最多重试一次 | `TokenRecoveryService` 是 Guard 自动恢复和认证失效 owner；无 maFile 的直连刷新由 `refreshWorkflow` 在同一错误语义下删除 TokenStore；`uiServer` 只投影恢复状态，前端打开对应重登框；`steamAuthDiagnosticLog` 将失败以账号哈希和固定字段写入 `logs/steam_auth/diagnostics.jsonl` | 每账号自动恢复 single-flight；网络错误和不含明确 15 的普通 AccessDenied 不触发；自动凭据登录再次返回 `15` 时先清本地 password，再返回 `password_reentry_required/password_cleared=true`；诊断仅记录阶段、账号摘要、`eresult/code`、reason、恢复模式和 token 清理结果，不记录账号名、密码、token、maFile、Guard secret 或原始错误全文；日志写失败不得改变认证结果 | 2026-07-19 真实 8799 页面已从连接入口越过本地前置阶段，收到真实 Steam `code=15` 后诊断记录为 `login_key_invalid/token_cleared=true/recovery_mode=manual_login`，账号卡变为“登录过期”并打开手动重登框；同轮完整 116 项 Node 测试通过 |
| `inventory.refresh` | Steam/GC inventory -> `parseInventory()` -> processed snapshot -> `UiStateStore` snapshot pointer / `inventory_refreshed` -> `/api/snapshot/account` -> `loadSnapshotForAccount()` / `setRows()` | `refreshWorkflow` 先写 snapshot；`refreshRuntime` 在 Node EventEmitter 中产出事件，经 SSE 跨到浏览器 `EventSource` consumer；前端 `visibilitychange/pagehide` 是每页 SSE 生命周期 owner，隐藏页关闭、恢复可见时只按当前账号重建；当前客户端用 `stream_version=2` 握手，`/api/events` 对无版本旧客户端返回 204 终止 EventSource 自动重连 | snapshot 文件先落盘，但 `buildRefreshPayload()` 会吞掉 snapshot pointer 持久化错误，随后仍可能发 `inventory_refreshed`；手动请求可用 response rows，另一个/重连 consumer 可能按旧 pointer 读旧值。连接前置 `disconnect-others` 设 5 秒本地超时，避免连接池异常时永久锁住 40% overlay；它不替代 SSE 生命周期清理。旧客户端 204 兼容分支在所有受支持客户端均确认发送 v2 且运行态不再出现旧重连后可删除 | 2026-07-19 运行态追踪确认多个隐藏页自动重连 `/api/events` 并耗尽浏览器同源连接池；v2/204 RED-GREEN 路由测试通过，重启后 Established 从 6 降至 1；真实 `127.0.0.1:8799` 页面点击 LOVE“登录过期”直接打开重登弹窗且不再出现 disconnect-others 超时；完整 117 项 Node 测试通过，真实跨窗口新 rows 收敛仍未验证 |
...
| `node_sidecar/src/appAuthStore.js` | Domain / persistence | 本地 user/super-admin、Steam 账号绑定、账号级投影，以及 `accounts.json` 到 SQLite 的一次性迁移 | `uiServer.js` auth/account 路由、scoped stores、首次可写初始化 | SQLite app_user/role/session/steam_account/app_setting、viewer binding、余额元数据 | API super-admin override、`/api/accounts`、账号范围 gate、账号/余额/迁移测试 | 旧文件迁移只插入缺失账号，不能覆盖 SQLite 当前密码/资料/Guard；事务完成前不得删除旧文件，损坏输入必须保留。不要把它当 permission-code owner；empty viewer 会 allow-all，store 打开失败时 `uiServer` 另有 always-allow fallback；余额字段是投影 |
| `node_sidecar/src/accountStore.js` | Domain adapter | 对 Steam account 做按 viewer 裁剪的兼容 store；通过 `saveVerifiedCredentials` 暴露 Phase 1 已验证账号密码的专用写入口 | `uiServer.js`、refresh/craft/component 等服务 | active Steam account、账号明细裁剪、`AppAuthStore` 持久化 | 账号相关 API、真实账号动作服务 | 它是适配层，不是新的账号真相 owner；专用密码写不得退化为会覆盖资料或切换活动账号的通用 upsert |
| `node_sidecar/src/steamGuardCoexistService.js`、`steamGuardSteamAdapter.js`、`steamGuardTokenService.js`、`steamGuardImportService.js` | Auth / external account | 联合绑定内存状态机、唯一 AddAuthenticator 调用、Guard-only maFile 规范化/预检、显式同名覆盖、后端 TOTP/导出 | Steam Guard coexist/import/token routes、账号卡令牌管理、普通登录的后端本地 Guard 提交和批量导入 UI | 临时登录 token、候选 Guard、`steam_account.mafile_content` | `AppAuthStore.updateSteamGuard()`、专用 create/overwrite/clear 入口、安全 API/SSE 投影 | 批量导入不得登录或写 TokenStore；普通登录不得把本地动态码返回前端；不得恢复 finalize/remove/replace；真实 Steam App 共存流程仍需测试账号验证 |
| `node_sidecar/src/tokenRecoveryService.js`、`services/sessionPool.js` | Auth / recovery | 有本地 maFile 账号在 refresh token 缺失/拒绝后的 single-flight 自动登录、持久化、会话失效和单次业务重试；无 maFile 保持手动令牌登录 | 库存刷新、GC Session、Web inventory、market/account tools | `TokenStore`、账号凭据、Guard TOTP、session pool | refresh runtime、Web operation wrapper、UI reconnecting/needs_attention/登录过期弹窗 | 网络失败不能误触发恢复；无 maFile 不得发起账号密码自动登录；`InvalidPassword` 必须结合 refresh-token/password 模式解释；密码清除和 durable token write 失败都不得发成功；不可递归恢复 |
...
| `node_sidecar/src/logger.js`、`devFileLog.js` | Infrastructure / diagnostics | 保持既有控制台输出，并在 `CS2_DEV_FILE_LOG=1` 时追加脱敏的按日 JSONL；仅保留本地日期当天和前两天 | 所有 `DedupLogger` 调用方；环境变量只在进程启动时读取 | level/scope/message 诊断事件，不是业务真相 | 开发排障、定向日志测试、`logs/dev` | 默认关闭且不得用于正式用户；文件失败不能改变业务控制流；`code/eresult` 保留不代表外部错误已完成语义归类 |
...
- `last verified`: `2026-05-30`；Steam 钱包余额来源边界小范围补验为 `2026-06-06`；license/AppAuthStore owner 和 inventory propagation 定向静态复核为 `2026-07-13`；Web 库存网络前置检查和 ban SteamID fallback 定向测试为 `2026-07-15`。
- `validation method`: 静态切片整理；历史记录包括 client auth、account scope、component permission、UI permission state、raw fetch auth handling 等定向测试和 2026-06-06 余额 targeted tests。2026-07-13 只读核对 `resolveRequestAuth()`、`hasPermission()`、`requirePermission()`、license runtime、local super-admin、empty-viewer/store-failure account scope、前端 permission codes、refresh snapshot/event/consumer 入口和相关测试定位；2026-07-15 运行 `account-scope-route`、`web-inventory-bootstrap`、`web-inventory-route-logging`、`web-inventory-gc-source`，验证 precheck 响应不泄露外部错误明细以及 `steam_id` fallback；本地 live precheck 只返回 `{ok:true, reachable:true, reason:""}`。未真实点击 ban 检测或触发库存拉取。
...
- `scope/coverage`: 炼金页高风险链路包括选物品/配方队列、辅助选材 `/api/craft/assist-select`、产物预测 `/api/craft/predict-outcomes`、真实执行 `/api/craft/tradeup` 或 `/api/craft/tradeup-with-components`。真实执行返回会回写当前 UI 库存 rows。多账号汰换复用炼金 preset 和 assist/tradeup 接口，按账号切换 active account 并逐个执行；批量 preset popover 只编辑 preset 投影，但候选来源明确分为 `/api/simulation/tradeup/search-items` 的全部符合物品和当前 warehouse groups 的仓库快捷选择。目录项以 English `markethashname` 持久化、中文 `name` 展示，服务端按 English 原始名或中文 `alchemy_name` 都可匹配库存。辅料可以不存在或保留为 `count=0`；主料仍至少为 1，真正进入 assist selection 前会过滤数量 0 的材料。批量 preset 的主辅数量始终动态补齐为 10：修改任一侧都会重算另一侧；无辅料或辅料为 0 时主料固定为 10 且主料数量控件锁定，已选择的辅料仍可从 0 增加。craft-assist preset 先同步 localStorage，再按编辑顺序串行写入 `/api/ui-state/craft-assist-presets`，避免快速连续编辑时旧请求覆盖新状态。主炼金页的 craft-assist 编辑器是固定三栏弹窗：左栏保存主/辅材料草稿，中栏统一数据库和仓库候选，右栏以 `preview_only` 先取得与执行一致的输入再预测；保存方案的编辑按 ID 原位覆盖。`rarity_tag` 标签模式只持久化品级与可选目标，不持久化暂时停用的普通材料；空目标按相对磨损升序和资产 ID 稳定次序选前 10 件，填写目标则复用既有 default/approach solver 且忽略固定磨损偏移。主炼金页把普通、纪念品和 StatTrak 输入视为同一兼容集合，产物统一解析为普通版本；模拟页仍保留自己的 StatTrak 池语义。汰换模拟是非真实执行链路，使用 `/api/simulation/tradeup/*`，preset 同步到 localStorage 和 `/api/ui-state/tradeup-simulation-presets`；已保存 preset 的普通保存覆盖原名称，笔形入口单独重命名。模拟 picker 在 DOM 生成前移除受限/稀有度不匹配候选。材料槽选择是立即可见的 UI 真源，不依赖派生产物成功；相对磨损小于等于 predictor `below` 安全偏移边界 `1e-7` 时显式使用 `infinite`，其余仍沿用默认 `below`。材料变更会失效旧派生 rows/candidates，旧预测响应不得覆盖新选择；API throw 以及 outcomes missing/type/empty 都只写已转义 warning，不得移除已选材料。导出到快速选材时主/辅材料按显式 collection/slot 或 material row 顺序归类，English `name` 用于匹配，中文 `display_name` 用于展示，主/辅数量和相对磨损范围独立配置。
...
- `last verified`: `2026-05-30`；preset 保存/重命名、picker 过滤、快速选材导出、batch preset 编辑和可选辅料数量语义的直接影响范围为 `2026-07-15`；模拟材料即时投影、零/近零预测边界、失败 envelope 和旧请求 freshness 的直接影响范围为 `2026-07-17`；主炼金页三栏编辑器、品级标签、预览选材一致性、产物磨损换算和统一 StatTrak/纪念品兼容规则的直接影响范围为 `2026-08-04`。
- `validation method`: 静态切片整理；运行 batch craft assist/execution writeback、component batch route、craft component service、candidate service、frontend permission UI 定向测试；2026-07-15 运行 simulation/craft-assist/batch 直接与相邻测试，并在浏览器运行态验证混合搜索结果不渲染受限物品、已保存 preset 覆盖保存不弹重命名、独立重命名入口、中文导出主辅字段、batch 搜索分为目录与仓库两层、无辅料和已选辅料时均可为 0、主辅数量按 `7+3 -> 8+2 -> 9+1 -> 10+0` 动态补齐、辅料为 0 时锁定主料、反向恢复与重载持久态恢复。`/api/simulation/tradeup/search-items` 实际成功 envelope 为 `{ok, items[]}`，本轮消费字段包括 `markethashname/name/rarity/collection/is_tradeup_restricted/tradeup_restriction_reason`。2026-07-17 运行派生产物、页面状态、picker guardrail、保存提示、predictor 契约和完整真实浏览器 picker 交互测试；新增测试覆盖 `target_relative_wear` 为 `0`/`1e-7`、API throw、outcomes missing/type/empty、旧 success/failure response、旧派生 projection 清理和双栏 warning escape。2026-08-04 运行 `node_sidecar` 默认完整测试集和根目录 craft-assist/worker/predictor/component/UI 相关测试；在真实 `127.0.0.1:8787`、`1569x912` 和 `1120x720` 验证固定三栏、标签草稿停用/恢复、数据库品级锁定、仓库连续选择、空目标标签预测、产物绝对/相对磨损换算、方案网格无重叠和旧方案精确回填。跨栏拖动在最后增加鼠标会话兜底后只完成源码契约测试，受内置浏览器 URL 策略中断未取得新一轮运行态结果。未执行真实炼金或多账号执行。
...
- `scope/coverage`: 运行态核心文件包括 `csgo_skins.db`、`inventory_ui_state.json`、`login_keys.json`、`logs/processed_inventory`。这只是核心例子，不是完整清单；`accounts.json` 现在仅是首次升级的一次性迁移输入，迁移状态与无 viewer 的 active Steam 账号写入 SQLite `app_setting`，成功后删除旧文件；`machine_id.bin`、`client_config.json`、`client_license_state.json`、`schema_cache.json` 等运行态文件仍需按任务核对。账号/权限/Steam 账号数据共用 SQLite；`steam_account.password` 有明文字段，应用登录密码是 hash，session token 存 hash。Steam refresh token 当前实现写 `login_keys.json`。Steam API key 的代码约定路径是 `node_sidecar/data/steam_api_key.enc`；Windows 下用 DPAPI，非 Windows 回退明文。开发态可写运行态文件写项目根；打包态写 Electron `userData`，打包首启会从打包资源 seed `client_config.json`、`schema_cache.json`、`csgo_skins.db` 到 `userData`。
