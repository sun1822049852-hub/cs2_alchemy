...
const state = {
  ...
  currentAccountUsername: "", connectedUsername: "", rows: [], mode: "cards", includeHidden: false, searchText: "",
  ...
  expandedGroups: new Set(), selectedComponentId: "", showComponentItems: false, componentAutoShowItems: false,
...
const ui = {
  ...
  componentPanel: document.getElementById("componentPanel"), componentSelect: document.getElementById("componentSelect"), componentHint: document.getElementById("componentHint"), showComponentItems: document.getElementById("showComponentItems"),
...
function setNoAccountState() {
  ...
  state.componentAutoShowItems = false;
...
function rowsForComponentScope() { const selected = selectedComponentId(); if (selected) return state.component.item_map[selected] || []; if (state.showComponentItems) return state.rows; return state.rows.filter((x) => !String(x.casket_id || "").trim()); }
...
function refreshComponentControls() { const summaries = Object.values(state.component.summary_map || {}).sort((a, b) => { const an = String(a.name || "").toLowerCase(), bn = String(b.name || "").toLowerCase(); if (an !== bn) return an.localeCompare(bn); return assetIdNumber(a) - assetIdNumber(b); }); ui.componentSelect.replaceChildren(); const allOpt = document.createElement("option"); allOpt.value = ""; allOpt.textContent = "全部组件"; ui.componentSelect.append(allOpt); for (const s of summaries) { const id = String(s.component_id || "").trim(); if (!id) continue; const expected = Number(s.expected_count || 0), loaded = Number(s.loaded_count || 0), tail = expected > 0 ? `${loaded}/${expected}` : `${loaded}`; const o = document.createElement("option"); o.value = id; o.textContent = `${compactComponentName(s.name)} [${tail}]`; ui.componentSelect.append(o); } if (!summaries.length) { ui.componentSelect.disabled = true; ui.componentSelect.value = ""; state.selectedComponentId = ""; ui.componentHint.textContent = "0个/0件"; return; } ui.componentSelect.disabled = false; if (!Object.prototype.hasOwnProperty.call(state.component.summary_map, state.selectedComponentId)) state.selectedComponentId = ""; ui.componentSelect.value = state.selectedComponentId; const total = summaries.length, loadedItems = Object.values(state.component.item_map || {}).reduce((a, arr) => a + arr.length, 0); ui.componentHint.textContent = `${total}个/${loadedItems}件`; }
...
function render() { if (state.currentPage !== "inventoryPage") return; refreshSortArrowStyles(); ui.componentPanel.classList.toggle("hidden", state.mode !== "cards"); const {allRows, filteredRows, filterKey} = getFilterResult(); refreshCollectionMenu(allRows, {sourceKey: makeCollectionSourceKey()}); state.mode === "grouped" ? renderGrouped(filteredRows, allRows, filterKey) : renderCards(filteredRows, allRows); }
function setRefreshBusy(busy) { state.refreshing = !!busy; ui.refreshBtn.disabled = state.refreshing || state.accounts.length <= 0; ui.useAccountBtn.disabled = state.refreshing || state.accounts.length <= 0; ui.loginSaveBtn.disabled = state.refreshing; renderSavedAccounts(); }
...
function bindEvents() {
  ...
  document.querySelectorAll("input[name='mode']").forEach((node) => { node.onchange = () => { state.mode = document.querySelector("input[name='mode']:checked").value; state.page = 1; render(); }; });
  ...
  ui.componentSelect.onchange = () => { const selected = String(ui.componentSelect.value || "").trim(); state.selectedComponentId = selected; if (selected) { if (!state.showComponentItems) state.componentAutoShowItems = true; state.showComponentItems = true; ui.showComponentItems.checked = true; } else if (state.componentAutoShowItems) { state.showComponentItems = false; ui.showComponentItems.checked = false; state.componentAutoShowItems = false; } state.page = 1; render(); };
  ui.showComponentItems.onchange = () => { state.showComponentItems = !!ui.showComponentItems.checked; state.componentAutoShowItems = false; if (!state.showComponentItems) { state.selectedComponentId = ""; ui.componentSelect.value = ""; } state.page = 1; render(); };
