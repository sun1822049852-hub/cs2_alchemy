...
} = require("../src/services/inventoryDisplayImageEnrichment");
...
async function test_internal_runtime_enrichment_targets_latest_snapshot_display_only_rows_missing_images() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "cs2-sidecar-display-image-enrichment-"));
...
(async () => {
  await test_internal_runtime_enrichment_targets_latest_snapshot_display_only_rows_missing_images();
  console.log("inventory-display-image-enrichment tests passed");
})().catch((err) => {
  console.error(err);
  process.exit(1);
});
