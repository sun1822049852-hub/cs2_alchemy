...
const BASELINE_TAG = "anchor-20260506_100641-preset-028-024-fast-optin";
const STAMP = new Date().toISOString().replace(/[-:]/g, "").replace(/\.\d{3}Z$/, "Z");
const PREFIX = `train-hunting-028-024-${BASELINE_TAG}-offline-final-${VARIANT}`;
const JSON_PATH = path.join(OUT_DIR, `${PREFIX}-${STAMP}.json`);
const MD_PATH = path.join(OUT_DIR, `${PREFIX}-${STAMP}.md`);
const PROFILE_PREFIX = `train-hunting-028-024-${BASELINE_TAG}-offline-profile-${VARIANT}`;
...
const SCRIPT_TIMEOUT_MS = Math.max(1000, Math.trunc(Number(process.env.CRAFT_ASSIST_BENCH_SCRIPT_TIMEOUT_MS) || 900000));
...
function findPreset(value) {
  ...
  if (normalizePresetName(value.name) === normalizePresetName(PRESET_NAME)) return value;
...
function buildPayload() {
  ...
  return {
    username,
    target_wear_raw: asString(preset.target_wear_raw || "0.24"),
...
async function main() {
  ...
  const artifact = {
    ...
    limitations: [
      "Offline route-equivalent verification, not an HTTP request through uiServer authentication and worker-pool timeout handling.",
      `The script sets long shard/prefilter timeout environment variables before service modules are loaded and enforces SCRIPT_TIMEOUT_MS=${SCRIPT_TIMEOUT_MS}.`,
...
function runWithScriptTimeout() {
  let timer = null;
  const timeoutPromise = new Promise((_, reject) => {
    timer = setTimeout(() => {
      const err = new Error(`SCRIPT_TIMEOUT_MS exceeded: ${SCRIPT_TIMEOUT_MS}`);
      err.code = "SCRIPT_TIMEOUT";
      reject(err);
    }, SCRIPT_TIMEOUT_MS);
  });
  return Promise.race([main(), timeoutPromise]).finally(() => {
    if (timer) clearTimeout(timer);
  });
}

runWithScriptTimeout().catch((err) => {
  const artifact = {
    status: err && err.code === "SCRIPT_TIMEOUT" ? "TIMEOUT" : "BLOCKED",
    timestamp: new Date().toISOString(),
    variant: VARIANT,
    error: String(err && err.stack || err),
    env_timeouts: {
  ...
  console.error(JSON.stringify({status: artifact.status, variant: VARIANT, error: artifact.error, json: JSON_PATH, md: MD_PATH}, null, 2));
  process.exit(2);
});
